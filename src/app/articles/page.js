...
import articles from "@/data/articles"; // Pastikan path ini benar
...
export default function ArticlesPage() {
  ...
  const filteredArticles = articles.filter((article) => {
    ...
    const matchesCategory = filter === "All" || (article.category && article.category.includes(filter)); // Asumsi ada field category, kalau tidak ada, hapus bagian ini
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-teal-50 animate-gradient relative overflow-hidden">
      ...
      {/* Decor */}
      <div className="absolute top-0 left-0 w-full h-[500px] bg-gradient-to-b from-white/60 to-transparent pointer-events-none"></div>

      <div className="max-w-6xl mx-auto px-6 pt-36 pb-32 relative z-10">
        
        {/* HEADER SECTION */}
        <div className="text-center mb-16 animate-fade-in-down">
          <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-white/60 backdrop-blur border border-white/50 text-indigo-600 text-sm font-bold mb-6 shadow-sm">
            <BookOpen size={18} />
          ...
          <div className="relative max-w-lg mx-auto group">
             ...
             <input 
                ...
                className="w-full pl-14 pr-6 py-4 rounded-full bg-white/80 backdrop-blur-xl border border-white/60 shadow-xl focus:ring-4 focus:ring-teal-100 focus:border-teal-300 outline-none transition-all text-gray-700 placeholder:text-gray-400 text-lg"
             />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-in-up">
            {filteredArticles.map((a, idx) => (
              <Link 
                key={a.id} 
                href={`/articles/${a.id}`}
                className="group glass-card rounded-[2rem] overflow-hidden hover-3d flex flex-col h-full"
              ...
              >
                ...
                <div className="p-8 flex-1 flex flex-col">
                   <div className="flex items-center gap-3 text-xs font-bold text-gray-400 mb-3">
                      <span className="bg-white/60 px-2 py-1 rounded-lg border border-white/50 text-indigo-500">
                        {a.category || "General"}
                      </span>
           ...
           <div className="text-center py-20">
              <div className="glass-panel inline-block p-8 rounded-3xl">
                 ...
                 <button onClick={() => setSearchQuery("")} className="mt-2 text-teal-600 font-bold hover:underline">Clear Search</button>
              </div>
           </div>
        )}
