...
export default function WelcomeModal({ onNameSubmit }) {
  ...
  useEffect(() => {
    ...
    if (!savedName) {
      setShow(true); // Munculkan modal jika belum ada nama
    } else {
      // Kalau sudah ada, kirim ke parent component
      if (onNameSubmit) onNameSubmit(savedName);
    }
  }, []);
  ...
  const handleSave = () => {
    ...
    // Kirim nama ke Home agar tampilan berubah
          ...
          {step === 1 ? (
            ...
            <div className="animate-fade-in-up">
              ...
              <p className="text-gray-500 mb-8 leading-relaxed">
                MindEase adalah ruang amanmu untuk beristirahat, bercerita, dan tumbuh. Sebelum kita mulai, boleh kami mengenalmu?
              </p>
              <button
                onClick={() => setStep(2)}
            ...
            <div className="animate-fade-in-up">
              ...
              <button
                onClick={handleSave}
                disabled={!name.trim()}
                className="w-full py-4 bg-teal-600 text-white rounded-xl font-bold hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-teal-200"
              >
                Mulai Perjalanan 🚀
              </button>
            </div>
